...
(function(exports, $){
	...
	function layer_Popup_Js() {
		var scroll_Y; // 스크롤값 담을 변수.
		var dim_Class_name = '.layer-dim-dark'; // 딤드 클래스.
		var $contents_All_Wrap = $('.content-parent-wrap'); // 컨텐츠 감싸고 있는 영역.
		var $parent_Tag = $('html, body');

		// bg open
		function bgOpen(bg_Class_Name) {
			$(bg_Class_Name).addClass('on');
		}
		// bg close
		function bgClose(bg_Class_Name) {
			$(bg_Class_Name).removeClass('on');
		}

		// layer popup open
		function layer_popuo_open(id) {
			var $id = $(id);
			scroll_Y = $(window).scrollTop();

			$parent_Tag.addClass('not-scroll');
			$contents_All_Wrap.css({
				'position': 'relative',
				'margin-top': -scroll_Y
			});
			bgOpen(dim_Class_name);
			//$id.addClass('on');
			$id.css('display', 'block').delay(200).queue(function(){
				$(this).addClass('on');
				$(this).dequeue();
			});
			// $('.layer-popup-over').perfectScrollbar();
		}

		// layer popup close
		function layer_popuo_close(popup) {
			var $popup = $('#' + popup);

			bgClose(dim_Class_name);
			//$popup.removeClass('on');
	    $popup.removeClass('on').delay(200).queue(function(){
	      ...
	      $(this).dequeue();
			...
			$parent_Tag.removeClass('not-scroll');
			$(window).scrollTop(scroll_Y);
		}

		$('.layer-open-btn').on('click', function(e){
			var $href = $(this).attr('href');

			e.preventDefault();
			layer_popuo_open($href);
		});

		$('.layer-close-btn').on('click', function(e){
			var $this_popup = $(this).parents('.layer-popup-wrap').attr('id');

			e.preventDefault();
			layer_popuo_close($this_popup)
		});
	}
